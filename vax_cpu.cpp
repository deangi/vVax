...
static uint32_t g_last_hb_instr = 0;
...
static uint32_t cpu_cur_mode() { return (g_st.psl >> 24) & 3u; }
...
static void do_casex(uint32_t sel, uint32_t base, uint32_t lim) {
  uint32_t r = sel - base;
  set_cmp_long(r, lim);
  if (r > lim) {
...
static void do_chmx(uint8_t opc, uint32_t arg) {
  // Study: Open SIMH vax_cpu1.c op_chm — simplified (no IS stack / full probes).
  ...
  mem_w32(tsp - 8, g_st.r[R_PC]);
  mem_w32(tsp - 4, g_st.psl);
  g_st.r[R_SP] = tsp - 12;
  g_stk[mode] = g_st.r[R_SP];
  g_st.psl = (mode << PSL_CUR_SHIFT) | (g_st.psl & PSL_IPL_MASK) | (cur << PSL_PRV_SHIFT);
  g_st.psl &= ~(PSL_N | PSL_Z | PSL_V | PSL_C);
...
static void ipr_write(uint32_t ipr, uint32_t value) {
  ...
  switch (ipr) {
    ...
    case 35: vax_console::txdb_wr(value); break;
    default: break;
  }
}
...
static void try_deliver_irq(uint16_t vector, uint8_t ipl) {
  ...
  // SIMH intexc: SCB bit 0 → interrupt stack (ISP is a virtual address).
  if (!psl_is()) {
    g_stk[psl_cur() & 3u] = g_st.r[R_SP];
    if (flags & 1u && g_isp) {
      g_st.psl |= PSL_IS;
      g_st.r[R_SP] = g_isp;
    } else if (g_stk[0]) {
  ...
  g_st.psl = (g_st.psl & ~PSL_IPL_MASK) | ((uint32_t)ipl << PSL_IPL_SHIFT);
  g_st.r[R_SP] -= 4;
  mem_w32(g_st.r[R_SP], old_psl);
  g_st.r[R_SP] -= 4;
  mem_w32(g_st.r[R_SP], old_pc);
  g_st.r[R_PC] = handler;
  g_st.irq_count++;
  if (g_irq_log_left > 0) {
    g_irq_log_left--;
...
static void raise_mmgt(uint32_t va, bool write) {
  ...
  // Exception: kernel mode, previous=old, keep IPL, clear CC. Do not raise IPL.
  g_st.psl = (g_st.psl & (PSL_IS | PSL_IPL_MASK)) | (old_cur << PSL_PRV_SHIFT);
  ...
  g_st.r[R_SP] -= 4;
  mem_w32(g_st.r[R_SP], va);  // p2 = VA
  g_st.r[R_SP] -= 4;
  mem_w32(g_st.r[R_SP], p1);  // p1 = MM_PARAM

  g_st.r[R_PC] = handler;
  g_mmgt_abort = true;

  if (g_mmgt_log_left > 0) {
    g_mmgt_log_left--;
    LOG("boot: %s VA=%08X wr=%u PC=%08X -> %08X p1=%u SP %08X→%08X",
        tag,
        (unsigned)va, write ? 1u : 0u, (unsigned)fault_pc,
        (unsigned)handler, (unsigned)p1,
        (unsigned)old_sp, (unsigned)g_st.r[R_SP]);
  }
}
...
static void service_interrupts() {
  ...
  if (vax_mscp::irq_pending()) {
    ...
    if (vec && vec < 0x400u) {
      ...
      if (g_st.irq_count != before) {
        vax_mscp::irq_clear();
        if (g_mscp_irq_logs || vax_mscp::host_online_wait()) {
          if (g_mscp_irq_logs) g_mscp_irq_logs--;
...
static void probe_rootopen() {
  ...
  static Probe probes[] = {
      {0x80019DA4u, 0x4Cu, "cpu_rootconf", 2, 0, true},
      {0x80229014u, 0x46Cu, "vfs_mountroot", 3, 0, true},
      {0x8001F0B4u, 0x19Cu, "raopen", 4, 0, true},
      {0x8001F6DEu, 0x26u, "ra_putonline", 4, 0, true},
      {0x8001E7FAu, 0x113u, "ra_putonline.body", 4, 0, true},
      {0x8001EC1Cu, 0x7Cu, "rx_putonline", 6, 0, true},
      {0x8002219Cu, 0x2Cu, "udaintr", 8, 0, false},
      {0x8001DC16u, 0x77u, "mscp_intr", 8, 0, false},
      {0x8001CF1Au, 0x73Eu, "mscp_dorsp", 8, 0, false},
      {0x8001F5C0u, 0x11Eu, "rronline", 6, 0, false},
      {0x8001D66Au, 0x53u, "mscp_worker", 8, 0, false},
      {0x801EDC2Cu, 0xF9u, "workqueue_enqueue", 8, 0, false},
      {0x801ED71Eu, 0x171u, "workqueue_worker", 8, 0, false},
      {0x801C717Au, 0xF6u, "tsleep", 8, 0, false},
      {0x801C751Eu, 0x4Fu, "wakeup", 8, 0, false},
      {0x80011128u, 0x25u, "gencngetc", 2, 0, false},
  };
  if (g_reset_probes) {
  ...
  uint32_t now = millis();
  if (now - g_root_hb_ms < 2000u) return;
  g_root_hb_ms = now;
...
static void exec_one() {
  ...
  if (g_logged_reloc) {
    g_instr_count++;
    // xxboot heartbeats, then kernel heartbeats after MAPEN so a hang
    // prints PC/PSL instead of looking like a freeze.
    if (!g_boot_elf_active && g_instr_count <= 500000u &&
        g_instr_count - g_last_hb_instr >= 100000u) {
      g_last_hb_instr = g_instr_count;
      LOG("boot hb: PC=%08X SP=%08X instr=%u",
          (unsigned)g_st.r[R_PC], (unsigned)g_st.r[R_SP],
          (unsigned)g_instr_count);
    } else if (g_boot_elf_active && !vax_mmu::mapen() &&
               g_instr_count - g_last_hb_instr >= 5000000u) {
      // /boot load can run for minutes; 100k was a serial flood.
      g_last_hb_instr = g_instr_count;
      LOG("boot ld hb: PC=%08X SP=%08X instr=%u",
          (unsigned)g_st.r[R_PC], (unsigned)g_st.r[R_SP],
          (unsigned)g_instr_count);
    } else if (g_boot_elf_active && vax_mmu::mapen() &&
               g_instr_count - g_last_hb_instr >= 2000000u) {
      g_last_hb_instr = g_instr_count;
      LOG("kern hb: PC=%08X SP=%08X PSL=%08X IPL=%u SISR=%04X irq=%u instr=%u ticks=%u wait=%u warp=%u",
          (unsigned)g_st.r[R_PC], (unsigned)g_st.r[R_SP],
          (unsigned)g_st.psl, (unsigned)cur_ipl(),
          (unsigned)(g_sisr & 0xFFFEu),
          (unsigned)g_st.irq_count, (unsigned)g_instr_count,
          (unsigned)vax_clock::ticks(),
          vax_mscp::host_online_wait() ? 1u : 0u,
          (unsigned)g_warp_fires);
    }
  }
  ...
  g_last_op_pc = op_pc;
  uint8_t op = fetch8();
  if (g_trace_left > 0 && op != 0x90 && op != 0xF5) {
    g_trace_left--;
  ...
  switch (op) {
    ...
    case 0x02: {  // REI — pop PC, then PSL; switch KSP/ISP like SIMH op_rei
      ...
      if (boot_entry) {
        ...
        LOG("boot: REI -> low PC=%08X (bad ELF e_entry/hoppabort?)", (unsigned)npc);
      }
      return;
    }
    ...
    case 0x04: {  // RET — frame matches SIMH/VAX: (FP)=cond, (FP)+4=SPA/S/mask/PSW
      ...
      g_st.r[R_AP] = mem_r32(fp + 8);
      g_st.r[R_FP] = mem_r32(fp + 12);
    ...
    case 0xF5: {  // SOBGTR
      ...
      if (!idx.ok) return;
      uint32_t a = idx.is_reg ? g_st.r[idx.reg] : mem_r32(idx.addr);
      // NetBSD delay() = cpu_vups * ms; uba/qe probe timeouts are 1e6+.
      // NEXTUNIT wrap (V0.6.26) ended the GUS scan in 3 commands — do not
      // keep a 20k-iteration cap or uba probes sit in SOBGTR @ 0x8001043E.
    ...
    case 0xFB: {  // CALLS — SIMH/VAX frame with condition-handler longword at (FP)
      ...
      g_st.r[R_SP] -= 4;
      mem_w32(g_st.r[R_SP], narg);
      uint32_t new_ap = g_st.r[R_SP];
    ...
    case 0x8F: {  // CASEB
      ...
      do_casex(sel.value & 0xFF, base.value & 0xFF, lim.value & 0xFF);
      return;
    }
    ...
    case 0xAF: {  // CASEW
      ...
      Opnd lim = decode_opnd(ACC_R, 2);
      if (!sel.ok || !base.ok || !lim.ok) return;
      do_casex(sel.value & 0xFFFF, base.value & 0xFFFF, lim.value & 0xFFFF);
      return;
    }
    ...
    case 0xCF: {  // CASEL
      ...
      if (!sel.ok || !base.ok || !lim.ok) return;
      do_casex(sel.value, base.value, lim.value);
      return;
    }
